...
uint16_t journal_find_eeaddr(uint8_t index, int8_t item_index) {
    ...
    journal_type_pos_t *journal_position = &journal_header.journal_type_pos[index];
    if (item_index == -1) {
        if (++journal_position->current >= journal_position->max) {
            journal_position->current = 0;
        }
        item_index = (int8_t) journal_position->current;
    }
